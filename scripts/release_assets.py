...
TARGETS = {
    'aarch64-apple-darwin': ('macos-arm64', ['.dmg', '.app.tar.gz', '.app.tar.gz.sig']),
    'x86_64-apple-darwin': ('macos-x64', ['.dmg', '.app.tar.gz', '.app.tar.gz.sig']),
    'x86_64-pc-windows-msvc': ('windows-x64', ['.exe', '.exe.sig', '.msi', '.msi.sig']),
    'x86_64-unknown-linux-gnu': ('linux-x64', ['.deb', '.rpm', '.AppImage', '.AppImage.sig']),
}
...
def prepare():
    ...
    for key, platform, suffix in [
        ...
        signature = (OUT / (name + '.sig')).read_text().strip()
        if not signature:
            raise RuntimeError('Empty updater signature')
        platforms[key] = {'signature': signature, 'url': base + quote(name)}
    manifest = {'version': VERSION, 'notes': 'Bastet Agent Sync — see the five-language release notes.',
    ...
    installer = (ROOT / 'scripts/install-linux.sh').read_text().replace('@VERSION@', VERSION)
    (OUT / 'install-linux.sh').write_text(installer)
    hashes = [f'{hashlib.sha256(p.read_bytes()).hexdigest()}  {p.name}' for p in sorted(OUT.iterdir())
              if p.is_file() and p.name not in ('SHA256SUMS', 'release-notes.md')]
    (OUT / 'SHA256SUMS').write_text('\n'.join(hashes) + '\n')
    rows = ['| Platform / 平台 | Download / 下載 |', '| --- | --- |']
    for label, platform, suffix in [
        ('macOS 12+ Apple Silicon', 'macos-arm64', '.dmg'),
        ('macOS 12+ Intel', 'macos-x64', '.dmg'),
        ('Windows 10/11 x64 (recommended)', 'windows-x64', '.exe'),
        ('Windows x64 MSI', 'windows-x64', '.msi'),
        ('Ubuntu 22.04+ / Debian 12+ x64', 'linux-x64', '.deb'),
        ('Fedora-family x64', 'linux-x64', '.rpm'),
        ('Linux x64 portable', 'linux-x64', '.AppImage'),
    ]:
        name = asset(platform, suffix)
    ...
    notes = (ROOT / 'docs/RELEASE_NOTES.md').read_text().replace('@VERSION@', VERSION).replace('@DOWNLOADS@', '\n'.join(rows))
    (OUT / 'release-notes.md').write_text(notes)
    print(f'Validated {len(hashes)} assets and four updater targets')

def draft():
    prepare()
    # Never overwrite or amend an existing release, including an earlier draft.
    ...
    subprocess.run(['gh', 'release', 'create', TAG, '--repo', REPO, '--target', sha,
                    '--title', f'🐈 Bastet Agent Sync {TAG}', '--draft', '--notes-file', str(OUT / 'release-notes.md'),
                    *[str(p) for p in sorted(OUT.iterdir()) if p.name != 'release-notes.md']], check=True)
