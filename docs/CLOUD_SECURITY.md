...
M3 is partially implemented. This preview includes OAuth and credential-store code, encrypted Drive API operations and an isolated GUI encryption check. It does **not** pass the M3 real-account/two-device gate. Start sync remains disabled. Native agent data is never uploaded by the current GUI.

## Connection boundary

The distributor supplies a Google **Desktop app** OAuth client through `BASTET_GOOGLE_CLIENT_ID` and, when required by that client, `BASTET_GOOGLE_CLIENT_SECRET` at build time. Desktop client metadata is distributed with the binary and cannot authenticate the application itself. No product client is committed or configured in the default build. End users are not asked to create a Cloud project.
...
The GUI currently inventories/creates folders only. Binding a folder to a verified sync space, account identity display, Picker, device listing and the recovery-key wizard remain pending. Folder inventory is not a successful sync.
...
Each space uses a random 256-bit key. A `bas1_` recovery code encodes that full random key, not a human password or a password-derived key. Import rejects malformed codes. Existing keys are not silently overwritten. The library supports saving/recovering a key; the GUI diagnostic creates and discards an ephemeral key without touching the user's keychain. The actual recovery export/confirmation/import wizard is not enabled. A recovery code must ultimately be exchanged outside the shared Drive folder and never included in support reports. Without a retained key/recovery code, ciphertext is unrecoverable. Member revocation and key rotation are future work.
...
3. Complete Picker/shared-folder grants, the space/key binding and recovery wizard, and GUI orchestration of the implemented encrypted queue.
