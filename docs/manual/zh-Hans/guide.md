...
勾选 Agent Memory OS、保存并启动后，程序会自动导出、加密传送 Drive、备份及合并，无需手动处理 JSONL。提供暂停、立即同步及计数。ChatGPT Work 独立列出；它与其他 Agent 的原生任务／对话迁移尚未完成，会逐来源跳过，不再全面阻挡。同步包含私有记忆、删除与权限，仅与可信且持有恢复密钥的设备共享。两台实体电脑验收仍待完成。
...
已加入第七个来源，可选记忆目录或使用 AGENT_MEMORY_HOME。以正式 AMOS CLI 导出，再使用 JSONL 检查面板；支持版本 1–3、最多 1 MiB。预览版保留封包原文，但尚不自动导出、发布到 Drive 或导入实时数据库。本机导出可能包含私有记忆；删除与组织权限数据须先审核。
