...
미출시: 독립 저장소 그룹을 최대 3개까지 병렬 동기화합니다. 공유 저장소와 겹치는 경로는 순서대로 처리합니다. 같은 경로의 Claude／Claude Code 및 Codex／ChatGPT Work는 한 번만 동기화합니다. 각 소스에 대기·동기화 중·결과를 표시합니다. 일시 중지는 새 작업을 멈추고 진행 중인 작업의 안전한 종료를 기다립니다. 한 소스의 실패가 다른 그룹을 막지 않으며 실제 속도 향상은 아직 측정하지 않았습니다.
