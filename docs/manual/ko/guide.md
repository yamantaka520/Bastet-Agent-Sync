...
Agent Memory OS를 선택·저장하고 시작하면 내보내기, 암호화 Drive 전송, 백업 및 병합을 자동 실행합니다. 수동 JSONL 처리는 필요 없습니다. 일시 중지·지금 동기화·건수를 표시합니다. ChatGPT Work는 별도 소스지만 Work와 다른 Agent의 기본 작업 이전은 아직 지원하지 않아 건너뜁니다. 비공개 기억·삭제·권한도 포함되므로 복구 키를 가진 신뢰하는 장치와만 공유하세요. 실제 컴퓨터 두 대 검증은 아직 완료되지 않았습니다.
...
일곱 번째 소스로 추가했습니다. 기억 디렉터리 또는 AGENT_MEMORY_HOME을 지정하고 공식 AMOS CLI로 내보낸 JSONL을 검사할 수 있습니다. 버전 1–3, 최대 1 MiB를 지원합니다. 내용을 보존하는 미리보기이며 자동 내보내기, Drive 게시, 실행 중인 DB 가져오기는 아직 지원하지 않습니다. 비공개 기억이 포함될 수 있으며 삭제와 조직 권한 적용은 검토가 필요합니다.

[Adapter contract](../../AGENT_MEMORY_OS.md)

브라우저가 열리기 전 실패하면 설정 읽기, 저장된 로그인, 브라우저 실행 오류를 구분합니다. 개발 버전 업데이트 후 이전 프로세스를 종료하고 다시 여세요. 설정을 읽을 수 없으면 JSON을 다시 가져오세요. 자격 증명 저장소에서 읽기 검증을 통과해야 첫 단계가 완료됩니다.

## 🐈 0.2.0 · Historical

0.2.0은 이모지 앱/Drive/동기화 상태와 로고 아래 버전 정보를 표시합니다. 시작 버튼은 저장된 설정과 지원되지 않는 소스를 확인합니다. 실제 Agent 동기화 worker는 아직 제공되지 않습니다. 사이드바에서 업데이트 확인, 서명 검증 설치, 재시작을 할 수 있습니다. 업데이트 목록은 Release 게시 후 제공됩니다.
