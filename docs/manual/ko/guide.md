...
OAuth와 자격 증명 저장소 코드, 암호화 Drive API, 샘플 데이터 암호화 및 복구 검사를 구현했습니다. 기본 빌드에는 OAuth 클라이언트가 없어 로그인이 비활성화됩니다. 로컬 HTTP 테스트는 실제 Google 또는 두 컴퓨터 검증이 아닙니다. 암호화 큐는 격리된 두 복제본을 교환하고 분기와 재시도 ID를 보존합니다. 공간 및 키 안내, Picker, GUI 큐 제어는 아직 남아 있습니다.
