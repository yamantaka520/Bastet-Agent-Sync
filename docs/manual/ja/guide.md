...
0.3.1 はローカル会話と Agent Memory OS を同期します。ソースを選択、保存して開始します。手動 JSONL 出力は不要です。Claude／Claude Code はローカル開発会話、Codex／Work はローカル記録を共有します。Agy は SQLite、Grok と Pi はネイティブ会話ファイルを使用します。
