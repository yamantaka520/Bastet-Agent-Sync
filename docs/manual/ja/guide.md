...
OAuth と資格情報ストアのコード、暗号化 Drive API、サンプルデータの暗号化・復元確認を実装しました。標準ビルドには OAuth クライアントがなく、ログインは無効です。ローカル HTTP テストは Google や実機 2 台での検証ではありません。暗号化キューは隔離された 2 つのレプリカを交換し、分岐と再試行 ID を保持します。スペース・鍵のウィザード、Picker、GUI のキュー制御は未完了です。
