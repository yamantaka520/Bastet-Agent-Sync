...
勾選 Agent Memory OS、儲存並啟動後，程式會自動匯出、加密傳送 Drive、備份及合併，不需手動處理 JSONL。提供暫停、立即同步及計數。ChatGPT Work 獨立列出；它與其他 Agent 的原生工作／對話搬移仍未完成，會逐來源略過，不再全面阻擋。同步包含私人記憶、刪除與權限，僅與自己信任且持有恢復金鑰的裝置共用。兩台實體電腦驗收仍待完成。
...
已加入第七個來源，可選記憶目錄或使用 AGENT_MEMORY_HOME。以正式 AMOS CLI 匯出，再使用 JSONL 檢查面板；支援版本 1–3、最多 1 MiB。預覽版保留封包原文，但尚不自動匯出、發布到 Drive 或匯入即時資料庫。本機匯出可能包含私有記憶；刪除與組織權限資料須先審核。
