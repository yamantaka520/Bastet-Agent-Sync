...
Since 0.2.1, the application connects selection and Start to a real background worker. Users do not pick export files: Bastet invokes the installed `agent-memory` CLI automatically, encrypts snapshots for the configured Google Drive space, and invokes the official merge on receiving devices. The manual JSONL inspector is an optional advanced diagnostic.

## Setup and trust

Complete the Drive wizard, select Agent Memory OS, save, then Start. Other selected local adapters run independently; an individual source failure does not veto this source. Discovery honors the selected memory home or `AGENT_MEMORY_HOME`. CLI lookup uses PATH, the standard memory virtual environment, common user installs and source checkouts; a native executable picker handles custom installs. Credentials, the active database and its WAL are never raw-copied into Drive.
...
ChatGPT Work appears separately in the source list and uses shared local Codex rollout storage. The other local conversation adapters are implemented within the [documented boundaries](NATIVE_SESSIONS.md); this does not imply cloud task migration or complete native continuation. Official Work documentation separates local and cloud execution; no native cross-device import API was established in the reviewed pages: [Work setup](https://learn.chatgpt.com/docs/get-started-with-work), [local execution](https://learn.chatgpt.com/docs/enterprise/chatgpt-work-local-security).
